...
namespace Apex {
	...
	public:
		...
		virtual inline const std::unordered_map<std::string, Ref<Mesh>>& GetMeshes() const { return m_Meshes; }
		virtual inline const Ref<Mesh>& GetMesh(const std::string& meshId) const { return m_Meshes.find(meshId)->second; }
		virtual inline const std::vector<std::string>& GetLoadedTexturePaths() const { return m_LoadedTexturePaths; }
