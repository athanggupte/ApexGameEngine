...
layout(location = 10) uniform vec3 _altMatAlbedo;
...
void main()
{
	...
	vec3 F = fresnel_SchlickRoughness(clamped_dot(N, V), F0, roughness);
	vec2 envBRDF = texture(_EnvBRDFLUT, vec2(clamped_dot(N,V), roughness)).rg;
